...
/*add by panpeng for fp*/
#define CF_FP_DOWNLOAD 		3 
#define CF_FP_RELEASE  		2
#define CF_FP_UNKOWN		0

int is_release_cf_fp = CF_FP_UNKOWN;

 int read_cf_driver_status(void)
{
	printk("%s--> status : %d",__func__,is_release_cf_fp);

	return is_release_cf_fp;
}

EXPORT_SYMBOL(read_cf_driver_status);
...
#define CF_NAV_INPUT_UP						0x282 //0x280->0x282 
#define CF_NAV_INPUT_DOWN					0x283 //0x281->0x283
#define CF_NAV_INPUT_LEFT					0x281 //0x282->0x281
#define CF_NAV_INPUT_RIGHT					0x280 //0x283->0x280
#define CF_NAV_INPUT_CLICK					0x72
...
{
	int ret = 0;
		printk("ssan cdfinger_power_on in \n");
#ifdef POWER_GPIO
	...
	if(ret)
	{
		printk("enable regulato fail\n");
		return ret;
	}
		printk("ssan power on \n");
#endif
	mdelay(1);
	gpio_set_value(pdata->reset_num, 1);
	msleep(10);
...
{
		printk("ssan cdfinger_power_off in \n");
#ifdef POWER_GPIO
...
static irqreturn_t cdfinger_eint_handler(int irq, void *dev_id)
{
	...
	if((screen_status == 0) && (g_cdfingerfp_data->is_finger_down == true))
	{
		//CDFINGER_ERR("cdfinger_eint_handler schedule_work \n");
		g_cdfingerfp_data->is_finger_down = false;
		schedule_work(&g_cdfingerfp_data->unlock_work);
	}
	return IRQ_HANDLED;
}
...
static void cdfinger_wake_lock(struct cdfingerfp_data *pdata,int arg)
{
	...
	else
	{
		...
		__pm_wakeup_event(pdata->cdfinger_lock, msecs_to_jiffies(1000));
	}
}
...
static int cdfinger_report_key(struct cdfingerfp_data *cdfinger, unsigned long arg)
{
	...
	case KEY_LEFT:
		...
		break;
	case KEY_F11:
		/*not report CF_NAV_INPUT_CLICK=0x72 by qujiong*/
...
static long cdfinger_ioctl(struct file* filp, unsigned int cmd, unsigned long arg)
{
	...
	printk("ssan cdfinger_ioctl cmd=[%d] \n", cmd);
...
#endif
			is_release_cf_fp = CF_FP_RELEASE;
										...
										unsigned long event, void* data)
{
	...
	//CDFINGER_ERR("cdfinger_fb_notifier_callback.\n");
	...
	switch (blank) {
		case FB_BLANK_UNBLANK:
			...
			screen_status = 1;
			//g_cdfingerfp_data->is_finger_down = true;
			if (isInKeyMode == 0)
				cdfinger_async_report();
...
/*add for fingerprint unlock screen by qujiong*/
static void cdfinger_fb_notification_unlock(struct work_struct *work)
{
	//CDFINGER_ERR("ssan cdfinger_fb_notification_unlock  in \n");
...
static int cdfinger_probe(struct platform_device *pdev)
{
	...
	CDFINGER_ERR("ssan cdfinger_probe in \n");
	...
	if (status != 1) {  // id pin is high , cdfinger
		...
		CDFINGER_ERR("ssan cdfinger_support_id ok \n");
	status = misc_register(&st_cdfinger_dev);
	if (status) {
		CDFINGER_ERR("cdfinger misc register err%d\n",status);
		return -1;
	}
		CDFINGER_ERR("ssan st_cdfinger_dev ok \n");
	cdfingerdev->miscdev = &st_cdfinger_dev;
	mutex_init(&cdfingerdev->buf_lock);
	cdfingerdev->cdfinger_lock = wakeup_source_register(&cdfingerdev->cdfinger_dev->dev, "cdfinger wakelock");
	...
	/*add for fingerprint unlock screen by qujiong*/
	INIT_WORK(&cdfingerdev->unlock_work,cdfinger_fb_notification_unlock);
	cdfingerdev->is_finger_down = false;

	g_cdfingerfp_data = cdfingerdev;
	CDFINGER_ERR("ssan cdfinger_probe ok \n");
