 ...
 */
ff_log_level_t g_log_level = 1;//__FF_EARLY_LOG_LEVEL;
...
static long ff_ctl_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case FF_IOC_SYNC_CONFIG: {
        ...
        g_log_level = 1;//g_config->log_level;
        break;
    }
...
static int __init ff_ctl_driver_init(void)
{
    int err = 0;
    FF_LOGV("'%s' enter.", __func__);
    if(((strstr(saved_command_line, "androidboot.fp=focal9362_tas") == NULL)) || (strstr(saved_command_line,"androidboot.mode=charger") != NULL)) {
        pr_info("%s This FPM vendor is not focal.\n",__func__);
        return 0;
    }
