...
#define WAKELOCK_HOLD_TIME 1500 /* in ms */ /* Win modify from 500 to 1500 for retry verify 7 times */
...
struct gf_key_map maps[] = {
	...
	{ EV_KEY, GF_KEY_INPUT_WAKEUP },
	{ EV_KEY, GF_KEY_INPUT_HOMEPAGE },
...
static void gf_kernel_key_input(struct gf_dev *gf_dev, struct gf_key *gf_key)
{
	...
	if (GF_KEY_POWER == gf_key->key && (gf_dev->key_customer_define == 1))
	{
		input_report_key(gf_dev->input, key_input, gf_key->value);
		input_sync(gf_dev->input);
	}
	else if((GF_KEY_POWER == gf_key->key || GF_KEY_CAMERA == gf_key->key)
			&& (gf_key->value == 1)) {
...
static int __init gf_init(void)
{
	...
	if(((strstr(saved_command_line, "androidboot.fp=gxfp5288_hg1") == NULL) &&
		(strstr(saved_command_line, "androidboot.fp=gxfp5288_hh6") == NULL) &&
		(strstr(saved_command_line, "androidboot.fp=gxfp5288_sd6") == NULL) &&
		(strstr(saved_command_line, "androidboot.fp=gxfp3258_drg") == NULL) &&
		(strstr(saved_command_line, "androidboot.fp=gxfp5288_c1n") == NULL) &&
		(strstr(saved_command_line, "androidboot.fp=gxfp3208_b2n") == NULL) &&
		(strstr(saved_command_line, "androidboot.fp=gxfp5288_sd1") == NULL)) || (strstr(saved_command_line,"androidboot.mode=charger") != NULL)) {
		pr_info("%s This FPM vendor is not goodix.\n",__func__);
		return 0;
	}
