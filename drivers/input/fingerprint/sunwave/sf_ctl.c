...
#define SF_NAV_INPUT_UP						0x280 //0x280->0x282 
#define SF_NAV_INPUT_DOWN					0x281 //0x281->0x283
#define SF_NAV_INPUT_LEFT					0x282 //0x282->0x281
#define SF_NAV_INPUT_RIGHT					0x283 //0x283->0x280

/*add for fingerprint unlock*/
#define SF_FP_UNLOCK_REJECTION_TIMEOUT 1500
...
/*add for fingerprint unlock*/
void sf_fb_notification_unlock(struct work_struct *work)
{
	xprintk(KERN_INFO,"sf_fb_notification_unlock in.\n");
	mdss_prim_panel_fb_unblank(SF_FP_UNLOCK_REJECTION_TIMEOUT);
}

/*add by panpeng for fp test */
static struct proc_dir_entry *proc_entry = NULL;

extern int read_cf_driver_status(void);
static int proc_show_ver(struct seq_file *file,void *v)
{
	int cf_status  = 100;

	cf_status = read_cf_driver_status();
	
	if(2 == cf_status)
	{
		seq_printf(file,"[Vendor]:sunwave-FP9151Y\n");
	}

	if(0 == cf_status)
	{
		seq_printf(file,"[Vendor]:cdfinger-FPS980S\n");
	}
...
static irqreturn_t sf_ctl_device_irq(int irq, void *dev_id)
{
	...
	/*add for fingerprint unlock*/
	if(sf_ctl_dev.is_finger_down == true)
	{
		xprintk(KERN_INFO,"sf_ctl_device_irq schedule_work.\n");
		sf_ctl_dev.is_finger_down = false;
		schedule_work(&sf_ctl_dev.unlock_work);
	}
...
static int sf_ctl_report_key_event(struct input_dev *input, sf_key_event_t *kevent)
{
    ...
    switch (kevent->key) {
        ...
        case SF_KEY_UP:
            key_code = SF_NAV_INPUT_UP;
            break;

        case SF_KEY_LEFT:
            key_code = SF_NAV_INPUT_LEFT;
            break;

        case SF_KEY_RIGHT:
            key_code = SF_NAV_INPUT_RIGHT;
            break;

        case SF_KEY_DOWN:
            key_code = SF_NAV_INPUT_DOWN;
            break;
                                   ...
                                   unsigned long event, void *data)
{
    ...
    switch (blank) {
        ...
        case FB_BLANK_POWERDOWN:
			xprintk(KERN_ERR,"sf_fb_notification_callback enter\n");
			sf_ctl_dev.is_finger_down = true;
            sf_suspend();
            break;
	...
	xprintk(KERN_ERR,"sf_fb_notifier_callback exit\n");
    return retval;
}
...
#ifdef CONFIG_PM_WAKELOCKS
        wakeup_source_destroy(sf_ctl_dev.wakelock);
#endif
        sf_ctl_dev.free_gpio(&sf_ctl_dev);
...
static int sf_probe(sf_device_t *dev)
{
	...
	if (NULL == proc_entry)
	{
		printk("sf_fp Couldn't create proc entry!");
		proc_entry = NULL;
	}
	else
	{
		printk("sf_fp Create proc entry success!");
	}
	/*end panpeng */
    ...
    /*Initialize the unlock screen work*/
	INIT_WORK(&sf_ctl_dev.unlock_work,sf_fb_notification_unlock);
	sf_ctl_dev.is_finger_down = false;
...
static int sf_ctl_init_input(void)
{
    ...
    __set_bit(SF_NAV_INPUT_UP,     sf_ctl_dev.input->keybit);
    __set_bit(SF_NAV_INPUT_LEFT,   sf_ctl_dev.input->keybit);
    __set_bit(SF_NAV_INPUT_RIGHT,  sf_ctl_dev.input->keybit);
    __set_bit(SF_NAV_INPUT_DOWN,   sf_ctl_dev.input->keybit);
