...
static int fts_read_touchdata(struct fts_ts_data *data)
{
   ...
   // FTS_FUNC_ENTER();
    ...
    /*modify by shenwenbin for some devices resume have touch need wait 2-3 seconds 20190514 end*/
...
static int fts_ts_probe(struct spi_device *spi)
{
    ...
    spi->chip_select = 0;//add by shenwenbin for compat second TP 20190326
...
//DDV-7 cann't load TP driver in poweroff charging mode by wangyibo at 20190508 start
int bootmode_poweroff_charging = 0;

static int __init tp_get_bootmode(char *line)
{
	char saved_bootmode[32];

	memset(saved_bootmode, 0, sizeof(saved_bootmode));
	strlcpy(saved_bootmode, line, sizeof(saved_bootmode));
	pr_err("cmdline=%s bootmode=%s\n", line, saved_bootmode);

	if (!memcmp(saved_bootmode, "charger", strlen("charger")))
		bootmode_poweroff_charging = 1;
	else
		bootmode_poweroff_charging = 0;
	pr_err("[%s] bootmode_poweroff_charging=%d\n", __func__, bootmode_poweroff_charging);
	return 1;
}
__setup("androidboot.mode=", tp_get_bootmode);
//DDV-7 cann't load TP driver in poweroff charging mode by wangyibo at 20190508 end
...
//DDV-7 cann't load TP driver in poweroff charging mode by wangyibo at 20190508 start
    if (bootmode_poweroff_charging == 1) {
	pr_err("[%s] bootmode_poweroff_charging=%d\n", __func__, bootmode_poweroff_charging);
	    return ret;
    }
//DDV-7 cann't load TP driver in poweroff charging mode by wangyibo at 20190508 end
    ...
    if ( panel_hardware_id == 101 ){
            ret = spi_register_driver(&fts_ts_driver);
            if ( ret != 0 ) {
                FTS_ERROR("Focaltech touch screen driver init failed!");
            }
    }else
        FTS_INFO("It's NOT Focaltech touch screen!");
