...
void himax_log_touch_data(int start)
{
	...
	if (hx_touch_data->diag_cmd > 0) {
		...
		buf = kzalloc(print_size * sizeof(uint8_t), GFP_KERNEL);
...
bool himax_ts_diag_func(void)
{
	...
	if (dsram_type >= 1 && dsram_type <= 3) {
		...
		for (i = 0; i < x_channel * y_channel; i++) {
			...
			strscpy(&write_buf[i*strlen(temp_buf)], temp_buf, sizeof(write_buf));
		}
		...
		for (i = 0; i < x_channel; i++) {
			...
			strscpy(&write_buf[(i+x_channel * y_channel)*strlen(temp_buf)], temp_buf, sizeof(write_buf));
		}
...
static ssize_t himax_esd_cnt_read(struct file *file, char *buf, size_t len, loff_t *pos)
{
	size_t ret = 0;
	char *temp_buf;
	I("%s: enter, %d \n", __func__, __LINE__);
