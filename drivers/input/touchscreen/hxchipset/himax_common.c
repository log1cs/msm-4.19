...
/*gest_event mapping to gest_key_def*/
        /*modify by shenwenbin for open double tap wakeup 20190516*/
...
#else
/*modify by shenwenbin for compat second TP 20190326 begin*/
char *i_CTPM_firmware_name = "Himax_firmware.bin";
/*modify by shenwenbin for compat second TP 20190326 end*/
#endif
...
void himax_update_usb_state(int insert){
#ifdef HX_USB_DETECT_GLOBAL
	USB_detect_flag = insert;
#endif
}
...
#ifdef HX_P_SENSOR
	himax_proc_psensor_file = proc_create(HIMAX_PROC_PSENSOR_FILE, (S_IWUSR | S_IRUGO | S_IWUGO),
...
int himax_input_register(struct himax_ts_data *ts)
{
	...
	set_bit(KEY_WAKEUP, ts->input_dev->keybit); /*modify by shenwenbin for open double tap wakeup 20190516*/
#endif
#if defined(HX_SMART_WAKEUP)
...
/* SMWP */
	...
	} else if (hx_EA_event == length) {
		lcd_need_reset=1;
		I("[HIMAX TP MSG]: ESD event checked - ALL 0xEA.\n");
	}
#ifdef HX_ZERO_FLASH
...
#ifdef HX_AUTO_UPDATE_FW
	if (g_auto_update_flag) {
		cancel_work_sync(&ts->work_update);
