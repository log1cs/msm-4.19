...
static int fts_gesture_read_buffer(struct i2c_client *client, u8 *buf, int read_bytes)
{
    ...
    if (read_bytes <= I2C_BUFFER_LENGTH_MAXINUM) {
        ret = fts_i2c_read_8719(client, buf, 1, buf, read_bytes);
    } else {
        ret = fts_i2c_read_8719(client, buf, 1, buf, I2C_BUFFER_LENGTH_MAXINUM);
        remain_bytes = read_bytes - I2C_BUFFER_LENGTH_MAXINUM;
        for (i = 1; remain_bytes > 0; i++) {
            if (remain_bytes <= I2C_BUFFER_LENGTH_MAXINUM)
                ret = fts_i2c_read_8719(client, buf, 0, buf + I2C_BUFFER_LENGTH_MAXINUM * i, remain_bytes);
            else
                ret = fts_i2c_read_8719(client, buf, 0, buf + I2C_BUFFER_LENGTH_MAXINUM * i, I2C_BUFFER_LENGTH_MAXINUM);
...
int fts_gesture_readdata_8719(struct fts_ts_data *ts_data)
{
    ...
    ret = fts_i2c_read_reg_8719(client, FTS_REG_GESTURE_EN, &state);
    ...
    ret = fts_i2c_read_8719(client, buf, 1, buf, FTS_GESTRUE_POINTS_HEADER);
    if (ret < 0) {
...
void fts_gesture_recovery_8719(struct i2c_client *client)
{
    if ((ENABLE == gdouble_tap_enable) && (ENABLE == fts_gesture_data.active)) {
        ...
        fts_i2c_write_reg_8719(client, 0xD6, 0xff);
        fts_i2c_write_reg_8719(client, 0xD7, 0xff);
        fts_i2c_write_reg_8719(client, 0xD8, 0xff);
        fts_i2c_write_reg_8719(client, FTS_REG_GESTURE_EN, ENABLE);
    }
}
...
int fts_gesture_suspend_8719(struct i2c_client *client)
{
    ...
    for (i = 0; i < 5; i++) {
        ...
        fts_i2c_write_reg_8719(client, FTS_REG_GESTURE_EN, ENABLE);
        msleep(1);
        fts_i2c_read_reg_8719(client, FTS_REG_GESTURE_EN, &state);
        if (state == ENABLE)
            break;
    }
...
int fts_gesture_resume_8719(struct i2c_client *client)
{
    ...
    fts_gesture_data.active = DISABLE;
    for (i = 0; i < 5; i++) {
        fts_i2c_write_reg_8719(client, FTS_REG_GESTURE_EN, DISABLE);
        msleep(1);
        fts_i2c_read_reg_8719(client, FTS_REG_GESTURE_EN, &state);
        if (state == DISABLE)
            break;
