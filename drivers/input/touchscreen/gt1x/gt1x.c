...
#include <linux/irq.h>
#include "gt1x.h"
#include "gt1x_generic.h" //add by snow
...
char fih_touch_hlt[32]={0};
/*Snow add for proc callback function*/
static int gLcmOnOff = 0; //DoubleTapOptionAllowedModifyOnlyWhenLcmIsOn
...
extern bool hlt_vbus_rising;//add by snow
u32 gt1x_get_charger_status(void)
{
	GTP_DEBUG("[charger_status]:snow hlt_vbus_rising is %d\n",hlt_vbus_rising ? 1 : 0);
	if (hlt_vbus_rising)
		return 1;
	else
		return 0;
...
static void gt1x_ts_work_func(struct work_struct *work)
{
	...
	//i++;
	//GTP_ERROR("snow enter interrupt time is %d\n!",i);
	...
	//GTP_ERROR("snow enter gesture wakeup interrupt time is %d\n!",i);
	if (ret >= 0) {
		goto exit_work_func;
	}
#endif
...
/*Snow add for proc callback function*/
void touch_hlt_vendor_read(char *buf)
{
	sprintf(buf, "%s\n","GT1152");
	return;
}

void touch_hlt_fwimver_read(char *buf)
{
	return;
}

void touch_hlt_fwver_read(char *fwver)
{
  sprintf(fwver, "%s",fih_touch_hlt);
	return;
}

void touch_hlt_upgrade_read(char *buf)
{
	return;
}

void touch_hlt_upgrade_write(int flag)
{
	return;
}
...
int hlt_selftest_result_read(void)
{
	...
	free(result);
	pr_err("This is selftest result : ret = %d\n",ret);
	return ret;
}

static void touch_hlt_selftest_write(void)
{
	return;
}
/*Snow add for proc callback function*/
...
//	INIT_WORK(&gt1x_work, gt1x_ts_work_func);
	...
	/*Snow add for proc callback function*/
	tp_probe_success = 1;
	touch_cb.touch_vendor_read = touch_hlt_vendor_read;
	touch_cb.touch_tpfwimver_read = touch_hlt_fwimver_read;
...
static int __init gt1x_ts_init(void)
{
    if (strstr(saved_command_line, "androidboot.device=PL2") != NULL)
    {
        ...
        {
            GTP_ERROR("[snow] lcm is ctc exit gt1x init now");
            return 0;
        }
    }
	GTP_DEBUG_FUNC();
