...
#include "../../../platform/fih/fih_touch.h" //add by snow 
...
u16 pre_event = 0;
u16 pre_index = 0;
int gdouble_tap_enable;
extern void gt1x_input_report_key(void);
extern void gt1x_input_sync(void);
...
static ssize_t gt1x_debug_read_proc(struct file *file, char __user * page, size_t size, loff_t * ppos)
{
	...
	temp_buff = (char *)kmalloc(GTP_CONFIG_MAX_LENGTH * 2 * 5 + 250, GFP_KERNEL);
	if (!temp_buff) {
	...
	ptr += sprintf(ptr, "ProductID: GT%c%c%c%c\n", temp_data[0], temp_data[1], temp_data[2], temp_data[3]);	//18 bytes
	ptr += sprintf(ptr, "PatchID: %02X%02X%02X\n", temp_data[4], temp_data[5], temp_data[6]);		//16 bytes
	ptr += sprintf(ptr, "MaskID: %02X%02X%02X\n", temp_data[7], temp_data[8], temp_data[9]);		//15 bytes
	ptr += sprintf(ptr, "SensorID: %02X\n", temp_data[10] & 0x0F);									//13 bytes

	i = ptr-temp_buff;
	if (copy_to_user(page, temp_buff, i)) 
	{
...
static ssize_t gt1x_debug_write_proc(struct file *file, const char __user *buffer, size_t count, loff_t * ppos)
{
	...
	if (strcmp(mode_str, "init") == 0) {
		GTP_INFO("Init panel");   //echo init > proc/gt1x_debug
		gt1x_init_panel();
		return count;
	}
	...
	if (strcmp(mode_str, "int") == 0) {
		if (mode == 0) {
			GTP_INFO("Disable irq.");  //echo int 0 >  proc/gt1x_debug
			gt1x_irq_disable();
		} else {
			GTP_INFO("Enable irq.");
			gt1x_irq_enable();
...
s32 gt1x_send_cfg(u8 * config, int cfg_len)
{
	...
	while (retry++ < 5) {
		...
		if (!ret) {
			msleep(80);	/* at least 80ms, wait for storing config into flash. */
            mutex_unlock(&mutex_cfg);
...
{
	if (level) {
		if ( gt_pinctrl.ts_pinctrl && gt_pinctrl.int_out_high)
...
void gt1x_select_addr(void)
{
	...
	gt1x_int_output(gt1x_i2c_client->addr == 0x14);
	msleep(2);
...
#define RST_RETRY       5
    ...
    /* release hold of cpu and not release DSP*/
    retry = RST_RETRY;
    do {
        d[0] = 0x08;
        ret = gt1x_write_and_readback(0x4180, d, 1);
 ...
 * Return 0-succeed.
 */
extern char fih_touch_hlt[32]; //add by snow
//extern char *fih_touch_hlt;
s32 gt1x_read_version(struct gt1x_version_info * ver_info)
{
	...
	int i, retry = 3;
	u8 checksum = 0;
    //char fih_touch_hlt[32]; //add by snow
...
static s32 gt1x_wakeup_sleep(void)
{
	...
	if (gdouble_tap_enable) {
		...
		if(ret) {
			GTP_ERROR("Gesture wake up sleep failed.");
			return ret;
		} else {
	...
	gt1x_power_reset();
	GTP_ERROR("snow1 Wakeup by poweron");
	return 0;
#else
	GTP_ERROR("snow2 Wakeup by not poweron");
	int flag = 0;
	/* int port wakeup */
	while (retry++ < 2) {
...
void gt1x_power_reset(void)
{
	...
	for (i = 0; i < 5; i++) {
	GTP_ERROR("snow enter for i is %d\n",i);
...
//add by ZZDC@snow for release tp data begin
	if (CHK_BIT(pre_event, BIT_TOUCH)) {
#if GTP_ICS_SLOT_REPORT
		int i;
		int cycles = pre_index < 3 ? 3 : GTP_MAX_TOUCH;
		gt1x_input_report_key();
		for (i = 0; i < cycles; i++) {
			if (pre_index >> i & 0x01) {
				gt1x_touch_up(i);
			}
...
#endif
		gt1x_input_sync();
		GTP_DEBUG("Released Touch before suspend.");
		pre_index = 0;
		pre_event = 0;
	}
...
GTP_ERROR("[gesture] snow1 begin gesture suspend ,gdouble_tap_enable is %d\n",gdouble_tap_enable);	
	if (gdouble_tap_enable) {
GTP_ERROR("[gesture] snow1 enter gesture suspend ,gdouble_tap_enable is %d\n",gdouble_tap_enable);
		gesture_enter_doze();
		gt1x_irq_enable();
		gt1x_halt = 0;
	} else
...
//add by snow
void hlt_tp_lcm_suspend(void)
{
	 gt1x_suspend();
	 pr_info("[snow] %s sleep start\n",__func__);
}
EXPORT_SYMBOL(hlt_tp_lcm_suspend);
...
int gt1x_resume(void)
{
    ...
    if (!gdouble_tap_enable && !gt1x_halt)
        return 0;
#else
    if (!gt1x_halt)
        return 0;
...
void hlt_tp_lcm_resume(void)
{
	//SW4-HL-Touch-ImplementDoubleTap-00*_20170623
	pr_err("snow begin hlt resume\n");
	gt1x_resume();

	return;
}
EXPORT_SYMBOL(hlt_tp_lcm_resume);
