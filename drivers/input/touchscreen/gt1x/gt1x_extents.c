...
u8 gestures_flag[32]; /* gesture flag, every bit stands for a gesture */
...
static ssize_t gt1x_gesture_data_write(struct file *filp, const char __user * buff, size_t len, loff_t * off)
{
	...
	ret = copy_from_user(&gdouble_tap_enable, buff, 1);
	if (ret) {
		GTP_ERROR("copy_from_user failed.");
		return -EPERM;
	}

	GTP_DEBUG("gesture enabled:%x, ret:%d", gdouble_tap_enable, ret);
