...
static void touchProcess(pabovXX_t this)
{
	...
	struct input_dev *input_prx = NULL;
	struct input_dev *input_drx = NULL;
	struct input_dev *input_wifi = NULL;
	...
	if (this && pDevice) {
		...
		input_prx = pDevice->pbuttonInformation->input_prx;
		input_drx = pDevice->pbuttonInformation->input_drx;
		input_wifi = pDevice->pbuttonInformation->input_wifi;
		numberOfButtons = pDevice->pbuttonInformation->buttonSize;

		if (unlikely((buttons == NULL) || (input_prx == NULL) || (input_drx == NULL) || (input_wifi == NULL))) {
			...
			case IDLE: /* Button is being in far state! */
				if ((i & pCurrentButton->mask) == pCurrentButton->mask) {
					LOG_INFO("CS %d State=BODY.\n",
							counter);
					if (board->cap_channel_prx == counter) {
	                    input_report_key(input_prx, pCurrentButton->keycode, 1);
	                    input_sync(input_prx);
	                    //SLEEP(1);
	                    input_report_key(input_prx, pCurrentButton->keycode, 0);
	                    input_sync(input_prx);
					} else if (board->cap_channel_drx == counter) {
	                    input_report_key(input_drx, pCurrentButton->keycode, 1);
	                    input_sync(input_drx);
	                    //SLEEP(1);
	                    input_report_key(input_drx, pCurrentButton->keycode, 0);
	                    input_sync(input_drx);
					} else if (board->cap_channel_wifi == counter) {
	                    input_report_key(input_wifi, pCurrentButton->keycode, 1);
	                    input_sync(input_wifi);
	                    //SLEEP(1);
	                    input_report_key(input_wifi, pCurrentButton->keycode, 0);
	                    input_sync(input_wifi);
					}		
				...
				break;
			case ACTIVE: /* Button is being in 0mm! */
				if ((i & pCurrentButton->mask) == pCurrentButton->mask) {
					...
					LOG_INFO("CS %d State=IDLE.\n",
							counter);
					if (board->cap_channel_prx == counter) {
	                    input_report_key(input_prx, pCurrentButton->keycode_release, 1);
	                    input_sync(input_prx);
	                    //SLEEP(1);
	                    input_report_key(input_prx, pCurrentButton->keycode_release, 0);
	                    input_sync(input_prx);
					} else if (board->cap_channel_drx == counter) {
	                    input_report_key(input_drx, pCurrentButton->keycode_release, 1);
	                    input_sync(input_drx);
	                    //SLEEP(1);
	                    input_report_key(input_drx, pCurrentButton->keycode_release, 0);
	                    input_sync(input_drx);
					} else if (board->cap_channel_wifi == counter) {
	                    input_report_key(input_wifi, pCurrentButton->keycode_release, 1);
	                    input_sync(input_wifi);
	                    //SLEEP(1);
	                    input_report_key(input_wifi, pCurrentButton->keycode_release, 0);
	                    input_sync(input_wifi);
					}
...
{
	struct device_node *np = client->dev.of_node;
	u32 cap_channel_prx, cap_channel_drx, cap_channel_wifi;
	int ret;
	int irq_irq_gpio;

	//client->irq = of_get_gpio(np, 0);
	//begin xiaopei add irq gpio sec,irq_gpio = <&tlmm 67 0>;
	irq_irq_gpio = of_get_named_gpio(np, "gpio-irq-std", 0);
	if (gpio_is_valid(irq_irq_gpio)) {
		ret = gpio_request_one(irq_irq_gpio, GPIOF_DIR_IN, "gpio-irq-std");
		if (ret) {
			printk("xiaopei gpio_request_one failed \n");
		}
	} else {
		printk( "%s: Failed to get irq gpio\n", __func__);
	}
	client->irq = gpio_to_irq(irq_irq_gpio);
	//End xiaopei add gpio irq

	LOG_INFO("gpio read succ: %d irq_irq_gpio = %d\n", client->irq,irq_irq_gpio);
	pplatData->irq_gpio = client->irq;
	ret = of_property_read_u32(np, "cap,use_channel_prx", &cap_channel_prx);
	ret = of_property_read_u32(np, "cap,use_channel_drx", &cap_channel_drx);
	ret = of_property_read_u32(np, "cap,use_channel_wifi", &cap_channel_wifi);
	LOG_INFO("cap_channel_prx=%d,cap_channel_drx=%d,cap_channel_wifi=%d\n", 
		      cap_channel_prx,cap_channel_drx,cap_channel_wifi);
	pplatData->cap_channel_prx = (int)cap_channel_prx;
	pplatData->cap_channel_drx = (int)cap_channel_drx;
	pplatData->cap_channel_wifi = (int)cap_channel_wifi;
	...
	LOG_INFO("firmware name read succ: %s\n", pplatData->fw_name);
}
		...
		const char *buf, size_t count)
{
	...
	input_report_key(input_prx, pDevice->pbuttonInformation->buttons[0].keycode_release, 1);
	input_sync(input_prx);
	//SLEEP(1);
	input_report_key(input_prx, pDevice->pbuttonInformation->buttons[0].keycode_release, 0);
	input_sync(input_prx);
	
	input_report_key(input_drx, pDevice->pbuttonInformation->buttons[1].keycode_release, 1);
	input_sync(input_drx);
	//SLEEP(1);
	input_report_key(input_drx, pDevice->pbuttonInformation->buttons[1].keycode_release, 0);
	input_sync(input_drx);
	
	input_report_key(input_wifi, pDevice->pbuttonInformation->buttons[2].keycode_release, 1);
	input_sync(input_wifi);
	//SLEEP(1);
	input_report_key(input_wifi, pDevice->pbuttonInformation->buttons[2].keycode_release, 0);
	input_sync(input_wifi);
...
static ssize_t sarsense_enable_store(struct class *class,
    struct class_attribute *attr,const char *buf, size_t count)
{
    if (!strncmp(buf, "1", 1)) {
        LOG_DBG("enable sar sensor\n");
        capEnabled = 1;
    } else if (!strncmp(buf, "0", 1)) {
        LOG_DBG("disable sar sensor\n");
        capEnabled = 0;
    } else {
        LOG_DBG("unknown enable symbol\n");
    }
		...
		char *buf)
{
	...
	return snprintf(buf, 16, "FW Version:0x%02x\n", fw_version);
}
...
//add i2c runin test by xiaopei NHK_M690_A01-1100
...
static int abov_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (ret == 0) {
		//add i2c runin test by xiaopei NHK_M690_A01-1100
		i2c_devinfo_device_write("SAR:0;");
		return -ENODEV;
	}
	if (ret == 2) {
		isForceUpdate = true;
	...
	if (this) {
		...
		//begin xiaopei add no input report problem 2019-4-5
		this->irq = client->irq;//xiaopei add
		//End xiaopei add no input report problem 2019-4-5
		...
		if (pDevice) {
			...
			input_prx = input_allocate_device();
			if (!input_prx)
				return -ENOMEM;

			/* Set all the keycodes */
			__set_bit(EV_KEY, input_prx->evbit);
			__set_bit(pDevice->pbuttonInformation->buttons[0].keycode, input_prx->keybit);
			__set_bit(pDevice->pbuttonInformation->buttons[0].keycode_release, input_prx->keybit);
			pDevice->pbuttonInformation->buttons[0].state = IDLE;
			/* save the input pointer and finish initialization */
			pDevice->pbuttonInformation->input_prx = input_prx;
			input_prx->name = "ABOV Cap Touch prx";
			if (input_register_device(input_prx)) {
				LOG_INFO("add prx cap touch unsuccess\n");
				return -ENOMEM;
			}
			/* Create the input device */
			input_drx = input_allocate_device();
			if (!input_drx)
				return -ENOMEM;
			/* Set all the keycodes */
			__set_bit(EV_KEY, input_drx->evbit);
			__set_bit(pDevice->pbuttonInformation->buttons[1].keycode, input_drx->keybit);
			__set_bit(pDevice->pbuttonInformation->buttons[1].keycode_release, input_drx->keybit);
			pDevice->pbuttonInformation->buttons[1].state = IDLE;
			/* save the input pointer and finish initialization */
			pDevice->pbuttonInformation->input_drx = input_drx;
			/* save the input pointer and finish initialization */
			input_drx->name = "ABOV Cap Touch drx";
			if (input_register_device(input_drx)) {
				LOG_INFO("add drx cap touch unsuccess\n");
				return -ENOMEM;
			}
			/* Create the input device */
			input_wifi = input_allocate_device();
			if (!input_wifi)
				return -ENOMEM;
			/* Set all the keycodes */
			__set_bit(EV_KEY, input_wifi->evbit);
			__set_bit(pDevice->pbuttonInformation->buttons[2].keycode, input_wifi->keybit);
			__set_bit(pDevice->pbuttonInformation->buttons[2].keycode_release, input_wifi->keybit);
			pDevice->pbuttonInformation->buttons[2].state = IDLE;
			/* save the input pointer and finish initialization */
			pDevice->pbuttonInformation->input_wifi = input_wifi;
			/* save the input pointer and finish initialization */
			input_wifi->name = "ABOV Cap Touch wifi";
			if (input_register_device(input_wifi)) {
				LOG_INFO("add wifi cap touch unsuccess\n");
				return -ENOMEM;
			}
		}
		...
		ret = class_create_file(&capsense_class, &class_attr_capenable);
		if (ret < 0) {
			LOG_DBG("Create capenable file failed (%d)\n", ret);
			return ret;
		}
		...
		abovXX_sar_init(this);
		 //xiaopei default turn on sar sensor 2019-3-20
        ...
        INIT_WORK(&this->fb_notify_work, tp_fb_notifier_resume_work);
	    //Begin xiaopei add CONFIG_FB to resume and suspend Tp 
		#ifdef CONFIG_FB
		this->fb_notif.notifier_call = fb_notifier_callback;
		ret = fb_register_client(&this->fb_notif);
		if(ret) {
			printk("register fb_notifier failed. ret=%d\n", ret);
		}
		#endif 
	    //End xiaopei add 1-4-2018
		
		//add i2c runin test by xiaopei NHK_M690_A01-1100
		i2c_devinfo_device_write("SAR:1;");
...
static int abov_remove(struct i2c_client *client)
{
	...
	if (this && pDevice) {
		...
		input_unregister_device(pDevice->pbuttonInformation->input_prx);
		input_unregister_device(pDevice->pbuttonInformation->input_drx);
		input_unregister_device(pDevice->pbuttonInformation->input_wifi);
		//Begin xiaopei add 2019-4-30 
		#if defined(CONFIG_FB)
		if (fb_unregister_client(&this->fb_notif))
...
static int abov_suspend(struct device *dev)
{
	...
	pabovXX_t this = i2c_get_clientdata(client);
	printk("xiaopei abov abov_suspend\n");
	abovXX_suspend(this);
	return 0;
}
/***** Kernel Resume *****/
static int abov_resume(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	pabovXX_t this = i2c_get_clientdata(client);
	//printk("xiaopei abov abov_resume\n");
	abovXX_resume(this);
	return 0;
}
/*====================================================*/
...
//Begin xiaopei use CONFIG_FB to suspend and resume 
#ifdef CONFIG_FB
static void tp_fb_notifier_resume_work(struct work_struct *work)
{
	struct abovXX *ts =
        container_of(work, struct abovXX,fb_notify_work);
	int ret = 0;

	ret = abov_resume(ts->pdev);
	if (ret < 0)
		LOG_ERR("abov_resume faild! ret=%d", ret);
	return;
}
static int fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
    ...
    struct abovXX *ts =
		container_of(self, struct abovXX, fb_notif);
...
static irqreturn_t abovXX_interrupt_thread(int irq, void *data)
{
	...
	mutex_lock(&this->mutex);
	//LOG_INFO("abovXX_irq-capEnabled = %d\n",capEnabled);
	if (capEnabled) {
		if ((!this->get_nirq_low) || this->get_nirq_low(this->board->irq_gpio))
...
void abovXX_suspend(pabovXX_t this)
{
	if (this) {
		...
		write_register(this,ABOV_CTRL_MODE_REG,0x02);//sar sensor suspend
		disable_irq(this->irq);
	}
}
void abovXX_resume(pabovXX_t this)
{
	if (this) {
		...
		write_register(this,ABOV_CTRL_MODE_REG,0x00);//sar sensor resume
		enable_irq(this->irq);
	}
}
