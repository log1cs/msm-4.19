...
static const struct of_device_id dummy_flash_dt_match[] = {
	{.compatible = "qcom,camera-led-flash", .data = NULL},
	{}
};
 
static struct msm_flash_table dummy_i2c_flash_table;

static struct msm_flash_table *flash_table[] = {
	&dummy_i2c_flash_table,
};
...
void dummy_torch_brightness_set(struct led_classdev *led_cdev,
...
static struct led_classdev msm_torch_led[MAX_LED_TRIGGERS] = {
	{
		.name		= "torch-light2",
		.brightness_set	= dummy_torch_brightness_set,
		.brightness	= LED_OFF,
	},
};

static int32_t dummy_torch_create_classdev(struct platform_device *pdev,
				void *data)
{
	int32_t rc = 0;
	int32_t i = 0;
	...
	for (i = 0; i < fctrl->torch_num_sources; i++) {
		if (fctrl->torch_trigger[i]) {
			...
			CDBG("%s:%d dummy_torch_brightness_set for torch %d",
				__func__, __LINE__, i);
			dummy_torch_brightness_set(&msm_torch_led[i],
				LED_OFF);
...
static int32_t dummy_flash_i2c_init(
	struct msm_flash_ctrl_t *flash_ctrl,
...
static int32_t dummy_flash_i2c_release(
	struct msm_flash_ctrl_t *flash_ctrl)
{
	return 0;
}

static int32_t dummy_flash_i2c_write_setting_array(
	struct msm_flash_ctrl_t *flash_ctrl,
	struct msm_flash_cfg_data_t *flash_data)
{
	return 0;
...
static int32_t dummy_flash_init(
	struct msm_flash_ctrl_t *flash_ctrl,
	struct msm_flash_cfg_data_t *flash_data)
{
	...
	pr_err("flash_ctrl->flash_state = %d, flash_ctrl->flash_driver_type = %d", flash_ctrl->flash_state, flash_ctrl->flash_driver_type);
	pr_err("flash_driver_type = %d", flash_data->cfg.flash_init_info->flash_driver_type);
...
static int32_t dummy_flash_init_prepare(
	struct msm_flash_ctrl_t *flash_ctrl,
	struct msm_flash_cfg_data_t *flash_data)
{
#ifdef CONFIG_COMPAT
	...
	if (!is_compat_task()) {
		...
		for (i = 0; i < MAX_LED_TRIGGERS; i++) {
			flash_data_k.flash_current[i] =
				flash_data->flash_current[i];
			pr_err("flash_current[%d] = %d\n", i, flash_data_k.flash_current[i]);
			flash_data_k.flash_duration[i] =
				flash_data->flash_duration[i];
			pr_err("flash_duration[%d] = %d\n", i, flash_data_k.flash_duration[i]);
		}
		...
		if (copy_from_user(&flash_init_info,
			...
			pr_err("%d, copy_from_user failed\n", __LINE__);
			return -EFAULT;
		}
		return dummy_flash_init(flash_ctrl, &flash_data_k);
	}
	/*
	 ...
	 */
	return dummy_flash_init(flash_ctrl, flash_data);
#else
	struct msm_flash_cfg_data_t flash_data_k;
	struct msm_flash_init_info_t flash_init_info;
	int32_t i = 0;
...
static int32_t dummy_flash_prepare(
	struct msm_flash_ctrl_t *flash_ctrl)
{
	int32_t ret = 0;
#if 0
...
static int32_t dummy_flash_config(struct msm_flash_ctrl_t *flash_ctrl,
	void __user *argp)
{
	int32_t rc = 0;
	struct msm_flash_cfg_data_t *flash_data =
	...
	pr_err("Enter, cfg_type = %d\n", flash_data->cfg_type);

	switch (flash_data->cfg_type) {
	case CFG_FLASH_INIT:
		rc = dummy_flash_init_prepare(flash_ctrl, flash_data);
		break;
	...
	rc = dummy_flash_prepare(flash_ctrl);
	if (rc < 0) {
		pr_err("Enable/Disable Regulator failed ret = %d", rc);
		return rc;
	}
	...
	unsigned int cmd, void *arg)
{
	...
	case VIDIOC_MSM_FLASH_CFG:
		return dummy_flash_config(fctrl, argp);
	...
	case MSM_SD_SHUTDOWN:
		if (!fctrl->func_tbl) {
			...
			return dummy_flash_prepare(fctrl);
		}
...
static int32_t dummy_flash_get_dt_data(struct device_node *of_node,
	struct msm_flash_ctrl_t *fctrl)
{
	int32_t rc = 0;
	...
	pr_err("fctrl->flash_driver_type = %d", fctrl->flash_driver_type);
	...
	struct file *file, unsigned int cmd, void *arg)
{
	...
	case VIDIOC_MSM_FLASH_CFG32:
		...
		case CFG_FLASH_LOW:
		case CFG_FLASH_HIGH:
			pr_err("%s %d, (CFG_FLASH_OFF/CFG_FLASH_LOW/CFG_FLASH_HIGH)\n",__func__, __LINE__);
...
#endif
static int32_t dummy_flash_platform_probe(struct platform_device *pdev)
{
	int32_t rc = 0;
	...
	rc = dummy_flash_get_dt_data(pdev->dev.of_node, flash_ctrl);
	if (rc < 0) {
		pr_err("dummy_flash_get_dt_data failed\n");
		kfree(flash_ctrl);
		return -EINVAL;
	}
	...
	pr_err("flash sd name = %s", flash_ctrl->msm_sd.sd.entity.name);
	...
	if (flash_ctrl->flash_driver_type == FLASH_DRIVER_PMIC)
		rc = dummy_torch_create_classdev(pdev, flash_ctrl);
#endif
...
MODULE_DEVICE_TABLE(of, dummy_flash_dt_match);

static struct platform_driver dummy_flash_platform_driver = {
	.probe = dummy_flash_platform_probe,
	.driver = {
		.name = "qcom,camera-led-flash",
		.owner = THIS_MODULE,
		.of_match_table = dummy_flash_dt_match,
	},
};

static int __init dummy_flash_init_module(void)
{
	int32_t rc = 0;
	pr_err("Enter\n");
	rc = platform_driver_register(&dummy_flash_platform_driver);
	if (rc)
		pr_err("platform probe for dummy flash failed\n");

        pr_err("Exit, rc = %d\n", rc);
	return rc;
}

static void __exit dummy_flash_exit_module(void)
{
	platform_driver_unregister(&dummy_flash_platform_driver);
	return;
}
...
static struct msm_flash_table dummy_i2c_flash_table = {
	...
	.func_tbl = {
		.camera_flash_init = dummy_flash_i2c_init,
		.camera_flash_release = dummy_flash_i2c_release,
		.camera_flash_off = dummy_flash_i2c_write_setting_array,
		.camera_flash_low = dummy_flash_i2c_write_setting_array,
		.camera_flash_high = dummy_flash_i2c_write_setting_array,
		.camera_flash_query_current = NULL,
	},
};

module_init(dummy_flash_init_module);
module_exit(dummy_flash_exit_module);
MODULE_DESCRIPTION("DUMMY FLASH");
