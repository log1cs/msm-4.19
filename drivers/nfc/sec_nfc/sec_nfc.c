...
#define NHK_BOARDID_C11 3

int board_id_global;
int board_id_get_board_id(void)
{
	return board_id_global;
}

static int __init setup_boardid_global(char *str)
{

	board_id_global = (int)simple_strtol(str, NULL, 10);
	return 1;
}

__setup("androidboot.boardid=", setup_boardid_global);
...
static irqreturn_t sec_nfc_irq_thread_fn(int irq, void *dev_id)
{
	...
	__pm_wakeup_event(info->nfc_wake_lock, 2*HZ);
...
static int __sec_nfc_probe(struct device *dev)
{
	...
	int ret = 0;
#if defined(CONFIG_LONGCHEER_SDM660_PROJS)
	int board_id = board_id_get_board_id();

	if (NHK_BOARDID_C11 == board_id)
		return -1;
	...
	info->nfc_wake_lock = wakeup_source_register(dev, "nfc_wake_lock");
...
static int __sec_nfc_remove(struct device *dev)
{
	...
	wakeup_source_unregister(info->nfc_wake_lock);
