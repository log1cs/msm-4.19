...
void tfa_set_query_info(struct tfa_device *tfa)
{
	...
	default:
		pr_err("tfa98xx: unknown device type : 0x%02x\n", tfa->rev);
		_ASSERT(0);
		break;
	}
}
...
enum Tfa98xx_Error tfa98xx_get_mtp(struct tfa_device *tfa, uint16_t *value)
{
	...
	if ( TFA_GET_BF(tfa, PWDN) ) {
		pr_err("tfa98xx: PLL in powerdown\n");
		return Tfa98xx_Error_NoClock;
	}

	tfa98xx_dsp_system_stable(tfa, &status);
	if (status==0) {
		pr_err("tfa98xx: PLL not running\n");
		return Tfa98xx_Error_NoClock;
	}
...
enum Tfa98xx_Error tfa98xx_set_mtp(struct tfa_device *tfa, uint16_t value, uint16_t mask)
{
	...
	if (tfa->verbose) {
		pr_err("tfa98xx: MTP clock enabled.\n");
	}
	...
	if (tfa->verbose) {
		pr_err("tfa98xx: MTP clock disabled.\n");
	}
...
void tfa98xx_set_exttemp(struct tfa_device *tfa, short ext_temp)
{
	if ((-256 <= ext_temp) && (ext_temp <= 255)) {
		...
		pr_err("tfa98xx: Using ext temp %d C\n", twos(ext_temp));
		TFA_SET_BF(tfa, TROS, 1);
		TFA_SET_BF(tfa, EXTTS, twos(ext_temp));
	} else {
		pr_err("tfa98xx: Clearing ext temp settings\n");
...
{
	if (tfa->in_use == 0) {
		pr_err("tfa98xx: device is not opened \n");
		return Tfa98xx_Error_NotOpen;
	}
...
tfa98xx_check_ic_rom_version(struct tfa_device *tfa, const unsigned char patchheader[])
{
	...
	if (checkaddress != 0xFFFF) {
		...
		if (error == Tfa98xx_Error_Ok) {
			if (value != checkvalue) {
				pr_err("tfa98xx: patch file romid type check failed [0x%04x]: expected 0x%02x, actual 0x%02x\n",
		...
		if ( checkvalue != 0xFFFFFF && checkvalue != 0) {
			...
			if ( revid != tfa->rev) {
				pr_err("tfa98xx: patch file device type check failed: expected 0x%02x, actual 0x%02x\n",
						tfa->rev, revid);
				return Tfa98xx_Error_Not_Supported;
			}
		}
...
enum Tfa98xx_Error dsp_msg(struct tfa_device *tfa, int length24, const char *buf24)
{
	...
	if(tfa->ext_dsp == 1) {
		...
		if(error == Tfa98xx_Error_Buffer_too_small) {
			...
			if (tfa->verbose) {
				pr_err("tfa98xx: Multi-message buffer full. Sending multi-message, length=%d \n", len);
			}
		...
		if(lastmessage == 1) {
			...
			if (tfa->verbose)
				pr_err("tfa98xx: Last message for the multi-message received. Multi-message length=%d \n", length);
	...
	if((tfa->verbose & 0x04)!=0) {
		pr_err("tfa98xx: DSP w [%d]: ", length);
		for(i=0; i<length; i++)
			pr_err("tfa98xx: 0x%02x ", (uint8_t)buf[i]);
		pr_err("\n");
	}
...
enum Tfa98xx_Error dsp_msg_read(struct tfa_device *tfa, int length24, unsigned char *bytes24)
{
	...
	/* DSP verbose has argument 0x04 */
	if((tfa->verbose & 0x04)!=0) {
		pr_err("DSP R [%d]: ", length);
		for(i=0; i<length; i++)
			pr_err("0x%02x ", (uint8_t)bytes[i]);
		pr_err("\n");
	}
                           ...
                           unsigned char data[])
{
	...
	if (num_bytes <= 0) {
		pr_err("Error: The number of READ bytes is smaller or equal to 0! \n");
		return Tfa98xx_Error_Fail;
	}
 ...
 */
enum Tfa98xx_Error tfa98xxSetCalibrateOnce(struct tfa_device *tfa)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;

	err = tfa98xx_set_mtp(tfa, 1<<TFA98XX_KEY2_PROTECTED_MTP0_MTPOTC_POS, 1<<TFA98XX_KEY2_PROTECTED_MTP0_MTPOTC_POS);
	if (err != Tfa98xx_Error_Ok) {
...
enum Tfa98xx_Error show_current_state(struct tfa_device *tfa)
{
	...
	if (tfa->tfa_family == 2 && tfa->verbose) {
		...
		pr_err("Current HW manager state: ");

		switch(manstate) {
			case 0: pr_err("power_down_state \n");
				break;
			case 1: pr_err("wait_for_source_settings_state \n");
				break;
			case 2: pr_err("connnect_pll_input_state \n");
				break;
			case 3: pr_err("disconnect_pll_input_state \n");
				break;
			case 4: pr_err("enable_pll_state \n");
				break;
			case 5: pr_err("enable_cgu_state \n");
				break;
			case 6: pr_err("init_cf_state \n");
				break;
			case 7: pr_err("enable_amplifier_state \n");
				break;
			case 8: pr_err("alarm_state \n");
				break;
			case 9: pr_err("operating_state \n");
				break;
			case 10: pr_err("mute_audio_state \n");
				break;
			case 11: pr_err("disable_cgu_pll_state \n");
				break;
			default:
				pr_err("Unable to find current state \n");
				break;
		}
	}
...
enum Tfa98xx_Error tfaRunSpeakerStartup(struct tfa_device *tfa, int force, int profile)
{
	...
	if (err) {
		pr_err("[%s] tfaContWriteFiles error = %d \n", __FUNCTION__, err);
		return err;
	}
	...
	if (err) {
		pr_err("[%s] tfaContWriteFilesProf error = %d \n", __FUNCTION__, err);
		return err;
	}
...
enum Tfa98xx_Error tfa_dev_start(struct tfa_device *tfa, int next_profile, int vstep)
{
	...
	pr_err("Active_profile:%s, next_profile:%s\n",
...
enum Tfa98xx_Error tfa_dsp_get_calibration_impedance(struct tfa_device *tfa)
{
	...
	if (tfa_dev_mtp_get(tfa, TFA_MTP_OTC)) {
		pr_err("Getting calibration values from MTP\n");
		...
		pr_err("Getting calibration values from Speakerboost\n");
...
enum Tfa98xx_Error dsp_partial_coefficients(struct tfa_device *tfa, uint8_t *prev, uint8_t *next)
{
	...
	for (eq=0; eq<NR_EQ; eq++) {
		...
		pr_err("eq_biquad_mask[%d] = 0x%.8x\n", eq, eq_biquad_mask[eq]);

		eq_offset += eq_biquads[eq];
	}

	pr_err("cost for writing all coefficients     = %d\n", old_cost);
	pr_err("cost for writing changed coefficients = %d\n", new_cost);
...
enum Tfa98xx_Error tfa_dev_set_state(struct tfa_device *tfa, enum tfa_state state)
{
	...
	case TFA_STATE_OPERATING:    /* Amp and Algo running */
		...
		if (tfa->verbose) {
			pr_err("FAIM disabled (err:%d).\n", err);
		}

		/* Synchonize I/V delay on 96/97 at cold start */
		if (tfa->sync_iv_delay) {
			if (tfa->verbose)
				pr_err("syncing I/V delay for %x\n",
				(tfa->rev & 0xff));
	...
	switch (item) {
		...
		case TFA_MTP_RE25_SEC:
			if((tfa->rev & 0xFF) == 0x88) {
				...
				pr_err("Error: Current device has no secondary Re25 channel \n");
			}
			break;
		case TFA_MTP_LOCK:
			break;
