...
const char *fw_name = "TFA9891.cnt";
...
static enum Tfa98xx_Error tfa98xx_tfa_start(struct tfa98xx *tfa98xx, int next_profile, int vstep)
{
	...
	if (trace_level & 8) {
		...
		dev_err(&tfa98xx->i2c->dev, "tfa98xx: tfa_dev_start(%d,%d) time = %lld us\n",
...
static int tfa98xx_input_open(struct input_dev *dev)
{
	...
	dev_err(tfa98xx->component->dev, "tfa98xx: opening device file\n");
...
static void tfa98xx_input_close(struct input_dev *dev)
{
	...
	dev_err(tfa98xx->component->dev, "tfa98xx: closing device file\n");
...
static int tfa98xx_register_inputdev(struct tfa98xx *tfa98xx)
{
	...
	dev_err(tfa98xx->component->dev, "tfa98xx: Input device for tap-detection registered: %s\n",
		input->name);
	tfa98xx->input = input;
	return 0;
				     ...
				     loff_t *ppos)
{
	...
	pr_err("tfa98xx: [0x%x] calib_done: %s", tfa98xx->i2c->addr, str);
				     ...
				     loff_t *ppos)
{
	...
	pr_err("tfa98xx: [0x%x] dsp_state : %s\n", tfa98xx->i2c->addr, str);
				     ...
				     size_t count, loff_t *ppos)
{
	...
	if (!strncmp(buf, start_cmd, sizeof(start_cmd) - 1)) {
		...
		ret = tfa98xx_tfa_start(tfa98xx, tfa98xx->profile, tfa98xx->vstep);
		mutex_unlock(&tfa98xx->dsp_lock);
		pr_err("tfa98xx: [0x%x] tfa_dev_start complete: %d\n",  tfa98xx->i2c->addr, ret);
	} else if (!strncmp(buf, stop_cmd, sizeof(stop_cmd) - 1)) {
		pr_err("tfa98xx: [0x%x] Manual triggering of dsp stop...\n",  tfa98xx->i2c->addr);
		mutex_lock(&tfa98xx->dsp_lock);
		ret = tfa_dev_stop(tfa98xx->tfa);
		mutex_unlock(&tfa98xx->dsp_lock);
		pr_err("tfa98xx: [0x%x] tfa_dev_stop complete: %d\n",  tfa98xx->i2c->addr, ret);
	} else if (!strncmp(buf, mon_start_cmd, sizeof(mon_start_cmd) - 1)) {
		pr_err("tfa98xx: [0x%x] Manual start of monitor thread...\n",  tfa98xx->i2c->addr);
				     ...
				     loff_t *ppos)
{
	...
	pr_err("tfa98xx: [0x%x] fw_state : %s", tfa98xx->i2c->addr, str);
				     ...
				     loff_t *ppos)
{
	...
	if (tfa98xx->tfa == NULL) {
		pr_err("tfa98xx: [0x%x] dsp is not available\n", tfa98xx->i2c->addr);
		return -ENODEV;
	}
	...
	if (buffer == NULL) {
		pr_err("tfa98xx: [0x%x] can not allocate memory\n", tfa98xx->i2c->addr);
		return -ENOMEM;
	}
	...
	if (error != Tfa98xx_Error_Ok) {
		pr_err("tfa98xx: [0x%x] dsp_msg_read error: %d\n", tfa98xx->i2c->addr, error);
		kfree(buffer);
		return -EFAULT;
	}
				     ...
				     size_t count, loff_t *ppos)
{
	...
	if ((msg_file->data[0] == 'M') && (msg_file->data[1] == 'G')) {
		...
		if (error != Tfa98xx_Error_Ok) {
			pr_err("tfa98xx: [0x%x] tfaContWriteFile error: %d\n", tfa98xx->i2c->addr, error);
			err = -EIO;
		}
	} else {
		error = dsp_msg(tfa98xx->tfa, msg_file->size, msg_file->data);
		if (error != Tfa98xx_Error_Ok) {
			pr_err("tfa98xx: [0x%x] dsp_msg error: %d\n", tfa98xx->i2c->addr, error);
			err = -EIO;
		}
	}
	mutex_unlock(&tfa98xx->dsp_lock);
...
#endif
		pr_err("tfa98xx: %d: vstep:%d, (control value: %d) - profile %d\n",
		       ...
		       struct snd_ctl_elem_info *uinfo)
{
	...
	pr_err("tfa98xx: vsteps count: %d [prof=%d]\n", tfacont_get_max_vstep(tfa98xx->tfa, profile),
			profile);
	return 0;
}
...
static void tfa98xx_tapdet_check_update(struct tfa98xx *tfa98xx)
{
	...
	if (!gpio_is_valid(tfa98xx->irq_gpio)) {
		...
		dev_err(tfa98xx->component->dev,
			"tfa98xx: Interrupt for tap-detection: %s (%d; 0x%x, %d)\n",
				enable? "enabled":"disabled",
				tfa98xx->tapdet_open, tfa98xx->tapdet_profiles,
				tfa98xx->profile);
		/*  enabled interrupt */
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	if (tfa98xx_container == NULL) {
		...
		pr_err("tfa98xx: container file already loaded...\n");
	...
	*/
	...
	if (tfa_is_cold(tfa98xx->tfa) == 0) {
		pr_err("tfa98xx: Warning: device 0x%.2x is still warm\n", tfa98xx->i2c->addr);
...
static void tfa98xx_dsp_init(struct tfa98xx *tfa98xx)
{
	...
	if (tfa98xx->dsp_fw_state != TFA98XX_DSP_FW_OK) {
		pr_err("tfa98xx: Skipping tfa_dev_start (no FW: %d)\n", tfa98xx->dsp_fw_state);
		return;
	}
	...
	if (tfa98xx->init_count < TF98XX_MAX_DSP_START_TRY_COUNT) {
		...
		if (ret == Tfa98xx_Error_Not_Supported) {
			...
			dev_err(&tfa98xx->i2c->dev,
						"tfa98xx: tfa_dev_start success (%d)\n",
						tfa98xx->init_count);
						...
						struct snd_soc_dai *dai)
{
	...
	kfree(basename);
        ...
        return 0; // fixed no voice sound issue, make a call and switch to speaker quickly
			     ...
			     struct snd_soc_dai *dai)
{
	...
	pr_err("tfa98xx: Requested rate: %d, sample size: %d, physical size: %d\n",
	...
	pr_err("tfa98xx: mixer profile:container profile = [%d:%d]\n", tfa98xx_mixer_profile, prof_idx);
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	dev_err(&tfa98xx->i2c->dev, "tfa98xx: %s: state: %d\n", __func__, mute);

	if (no_start) {
		pr_err("tfa98xx: no_start parameter set no tfa_dev_start or tfa_dev_stop, returning\n");
		return 0;
	}
...
static int tfa98xx_probe(struct snd_soc_component *component)
{
	...
	dev_err(component->dev, "tfa98xx component registered (%s)",
							tfa98xx->fw.name);
...
static int tfa98xx_parse_dt(struct device *dev, struct tfa98xx *tfa98xx,
	...
	rc = of_property_read_string(np, "nxp,fw-name", &fw_name);
	if(rc < 0)
		dev_err(dev, "tfa98xx: No fw_name provided. use Default %s\n",fw_name);
	else
		dev_err(dev, "tfa98xx: fw_name %s\n",fw_name);
			     ...
			     const struct i2c_device_id *id)
{
	...
	if ((no_start == 0) && (no_reset == 0)) {
		...
		switch (reg & 0xff) {
		case 0x72: /* tfa9872 */
			pr_err("tfa98xx: TFA9872 detected\n");
			...
			break;
		case 0x74: /* tfa9874 */
			pr_err("tfa98xx: TFA9874 detected\n");
			tfa98xx->flags |= TFA98XX_FLAG_MULTI_MIC_INPUTS;
			tfa98xx->flags |= TFA98XX_FLAG_CALIBRATION_CTL;
			tfa98xx->flags |= TFA98XX_FLAG_TDM_DEVICE;
			break;
		case 0x88: /* tfa9888 */
			...
			break;
		case 0x13: /* tfa9912 */
			pr_err("tfa98xx: TFA9912 detected\n");
			tfa98xx->flags |= TFA98XX_FLAG_MULTI_MIC_INPUTS;
			tfa98xx->flags |= TFA98XX_FLAG_TDM_DEVICE;
			/* tfa98xx->flags |= TFA98XX_FLAG_TAPDET_AVAILABLE; */
			break;
		case 0x94: /* tfa9894 */
			pr_err("tfa98xx: TFA9894 detected\n");
			tfa98xx->flags |= TFA98XX_FLAG_MULTI_MIC_INPUTS;
			tfa98xx->flags |= TFA98XX_FLAG_TDM_DEVICE;
			break;
		case 0x80: /* tfa9890 */
		...
		case 0x92: /* tfa9891 */
			...
			break;
		case 0x12: /* tfa9895 */
			pr_err("tfa98xx: TFA9895 detected\n");
			tfa98xx->flags |= TFA98XX_FLAG_SKIP_INTERRUPTS;
			break;
		case 0x97:
			pr_err("tfa98xx: TFA9897 detected\n");
			tfa98xx->flags |= TFA98XX_FLAG_SKIP_INTERRUPTS;
			tfa98xx->flags |= TFA98XX_FLAG_TDM_DEVICE;
                        SMARTAMP_READ_ID_FAIL;
			break;
		case 0x96:
			pr_err("tfa98xx: TFA9896 detected\n");
			tfa98xx->flags |= TFA98XX_FLAG_SKIP_INTERRUPTS;
			tfa98xx->flags |= TFA98XX_FLAG_TDM_DEVICE;
			break;
		default:
